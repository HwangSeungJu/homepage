...
- 새로운 번호의 생성   
SELECT NVL(MAX(no), 0) + 1 as no FROM qna; 
...
- 서브쿼리를 이용한 레코드 추가 
INSERT INTO qna(no, qname, title, content, passwd, wdate, gno)  
VALUES((SELECT NVL(MAX(no), 0) + 1 as no FROM qna), 
'왕눈이', '제목', '내용', '123', sysdate,
(SELECT NVL(MAX(gno), 0) + 1 FROM qna));
...
FROM(   
	...
	FROM(
		SELECT no, qname, title, content, passwd, viewcnt, wdate, gno, indent, anum 
		FROM qna  
		--where wname like '%g%'
